...
export function buildControls(host, engine, { onRandom } = {}) {
  ...
  Object.entries(PRESETS).forEach(([name, p]) => {
    ...
    b.addEventListener('click', () => { engine.cfg.camMode = 'orbit'; Object.assign(engine.cfg, p); surfBtn.classList.remove('on'); if (!engine.running) engine._draw(); });
    presetRow.appendChild(b);
  });
  body.appendChild(presetRow);

  // stand on the surface and look at the star (alien sky)
  const surfBtn = el('button', 'ctl-surface', 'Stand on the surface');
  surfBtn.addEventListener('click', () => {
    const on = engine.cfg.camMode !== 'surface';
    engine.surface(on);
    surfBtn.classList.toggle('on', on);
    surfBtn.textContent = on ? 'Return to orbit' : 'Stand on the surface';
  });
