...
import { classify, derived, physics, lifeAssessment } from './data.js';
...
export function buildNarrator(host) {
  ...
  const head = el('div', 'hud-head', `<span id="rep-title">Field Report</span>`);
  const lifeBtn = el('button', 'rep-life', 'Could life exist?');
  const modeEl = el('span', 'rep-mode', '…');
  head.append(lifeBtn, modeEl);
  panel.appendChild(head);
  const body = el('div', 'report-body');
  panel.appendChild(body);
  host.appendChild(panel);
  const titleEl = head.querySelector('#rep-title');

  const ai = new AIProvider();
  const ready = ai.detect().then((m) => { modeEl.textContent = m === 'echo' ? 'offline' : m; return m; });
  let ctrl = null;
  let current = null;

  async function stream({ title, system, facts, fallback }) {
    if (ctrl) ctrl.abort();
    ...
    ai.chat({
      ...
      echo: { speech: [fallback], thought: [] },
      onToken: (t) => { body.textContent += t; },
    }).catch(() => { body.textContent = fallback; });
  }

  lifeBtn.addEventListener('click', () => {
    if (!current) return;
    const a = lifeAssessment(current);
    lifeBtn.classList.add('on');
    stream({
      title: 'Astrobiology Assessment',
      system: 'You are an astrobiologist giving a grounded, sober verdict from a planet\'s real data: could life exist, and if so would it be carbon-based? Reason from temperature, atmosphere, the host star\'s activity, and chemistry. 3-4 sentences, scientific not fanciful, no lists or markdown. Be honest about what is plausible vs speculative.',
      facts: a.facts,
      fallback: a.sentences.join(' '),
    });
  });
  ...
  return {
    describe(p) {
      current = p;
      lifeBtn.classList.remove('on');
      const { sentences, facts } = buildSentences(p);
      stream({
        title: 'Field Report',
        system: 'You are a planetary scientist narrating the view from a starship window. Given a planet\'s real measured data, write 3 vivid, scientifically accurate sentences about what this world is and what it would be like to see. No preamble, no lists, no markdown.',
        facts,
        fallback: sentences.join(' '),
      });
    },
  };
}
